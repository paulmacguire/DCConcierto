...
<?php include('../templates/header.html'); ?>
    ...
    <?php
        ...
        $query = "SELECT * FROM eventos_propuestos WHERE REPLACE(LOWER(eventos_propuestos.artista), ' ', '_') = '$nombre' AND eventos_propuestos.estado_evento = 'Pendiente';";
...
<?php if (!empty($data)) {?>
  ...
  <h2> ¡Aquí puedes ver tus eventos pendientes!</h2><br>
      <div class="col align-self-center">
          <table class="table table-striped">
              ...
              <?php foreach ($data as $d){ ?>
                <form action="../consultas/cambiar_estado_evento.php" method="post">          
                <td><?php echo $d[1]; ?></td>
                <td><?php echo $d[2]; ?></td>
                <td><?php echo $d[7]; ?></td>
                <td><?php echo $d[4]; ?></td>
                ...
                <td><?php echo $d[8]; ?></td>
                <td><?php echo $d[9]; ?></td>
                <input name="id_evento" class="form-control" id="form3Example3" type="hidden" value="<?php echo $d[0] ?>"/>
                <td><input type="submit" value="Aceptar" name="accion"  class="btn btn-success"></td>
                <td><input type="submit" value="Rechazar" name="accion"   class="btn btn-danger"></td>
                </form>
                </tr>
              <?php } ?>                         
              </tbody>
